...
--
-- FUNCTION host.PROC_NAME
--
drop function if exists host.PROC_NAME
go
create function host.PROC_NAME(@procid int)
...
if object_id('host.TBjob') is null begin
  create table host.TBjob (
     ...
     DFesquema varchar(100) not null,
     DFmodulo varchar(100) not null,
     DFtarefa varchar(100) not null,
     DFprocedure as concat(DFesquema,'.jobtask__',DFmodulo,'__',DFtarefa),
     DFcod_empresa int null,
     DFid_usuario int null,
     DFid_referente sql_variant null,
     DFdata_execucao datetime not null default current_timestamp,
     DFguid_instancia uniqueidentifier null
  )

  create index IX__host_TBjob
      on host.TBjob (DFesquema, DFmodulo, DFtarefa)

  create index IX__host_TBjob__DFcod_empresa
      on host.TBjob (DFcod_empresa)

  create index IX__host_TBjob__DFguid_instancia
      on host.TBjob (DFguid_instancia)
end
...
--
drop procedure if exists host.jobtask__sandbox__exemplo_de_evento
go
create procedure host.jobtask__sandbox__exemplo_de_evento
  @comando varchar(10),
  @cod_empresa int = null,
  @id_usuario int = null,
